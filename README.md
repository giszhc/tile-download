...
- **自动识别瓦片像素尺寸**：按下载到的第一张图读出真实边长（256 / 512 / 128 都行），512 的高清瓦片不会错位，实测尺寸写在左下角信息条里
...
本地和局域网地址不走代理：`localhost` / `127.0.0.1` / `::1` / `*.local` 一律直连，系统绕过列表（`no_proxy`）里的主机同样直连——自建或内网瓦片服务属于这种情况。
