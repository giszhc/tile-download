...
- **支持代理**：自动读取 `HTTP_PROXY` / `HTTPS_PROXY`，https 走 CONNECT 隧道；本地 / 内网地址自动直连
- **随时中断**：`Ctrl+C` 立刻停止发起新请求，已下载的瓦片全部保留，预览页照常生成
- **自带预览页**：下载完在输出根目录生成单文件 `index.html`（Leaflet，CDN 引入），双击看拼合效果，右上角实时显示当前地图级别；自动识别瓦片像素尺寸（256 / 512 / 128），512 的高清瓦片不会错位
